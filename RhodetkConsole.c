    ...
    Tcl_Interp *interp;
{
    ...
    if (!consoleInitialized) {
      ...
      /*DR if (Tcl_GetStdChannel(TCL_STDIN) == NULL) {*/
      consoleChannel = Tcl_CreateChannel(&consoleChannelType, "console0",
					 (ClientData) TCL_STDIN, TCL_READABLE);
      if (consoleChannel != NULL) {
	Tcl_RegisterChannel(interp, consoleChannel);
	...
	Tcl_SetChannelOption(NULL, consoleChannel, "-encoding", "utf-8");
      ...
      /*DR if (Tcl_GetStdChannel(TCL_STDOUT) == NULL) {*/
	...
	Tcl_SetChannelOption(NULL, consoleChannel, "-encoding", "utf-8");
      ...
      /*DR if (Tcl_GetStdChannel(TCL_STDERR) == NULL) {*/
    ...
    Tcl_Interp *interp;			/* Interpreter to use for prompting. */
{
...
#else
    static char initCmd[] = "source ${.RootDir}/Src/console.tcl";
#endif
    ...
    /*DR...*/
    Tcl_CreateCommand(interp, "consoleinterp", InterpreterCmd,
		      (ClientData) info, (Tcl_CmdDeleteProc *) NULL);
    Tcl_SetVar(consoleInterp, ".RootDir", RootDir, TCL_GLOBAL_ONLY);
    Tcl_LinkVar(interp, ".BATCH", (char *) &Batch, TCL_LINK_INT);
#ifdef MACHINE_WINDOWS
    Tcl_SetVar(consoleInterp, ".WINDOWS", "1", TCL_GLOBAL_ONLY);
#else
    Tcl_SetVar(consoleInterp, ".WINDOWS", "0", TCL_GLOBAL_ONLY);
#endif /* MACHINE_WINDOWS */
    
    Tcl_Eval(consoleInterp, "source ${.RootDir}/Src/lensrc.tcl;"
	     "if [file readable ~/.lensrc] {source ~/.lensrc}");
    Tcl_Eval(consoleInterp,
	     "tk_setPalette ${.mainColor};"
	     "catch {font delete .NormalFont};"
	     "eval font create .NormalFont ${.normalFont};"
	     "eval font create .ConsoleFont ${.consoleFont};"
	     "option add *font .NormalFont userDefault");
    /*...DR */
    if (Tcl_Eval(consoleInterp, initCmd) == TCL_ERROR) {
	/* goto error; -- no problem for now... */
	error(consoleInterp->result);
    }
 ...
 */
	...
	/* ARGSUSED */
static void
ConsoleWatch(instanceData, mode)
    ClientData instanceData;		/* Device ID for the channel. */
    int mode;				/* OR-ed combination of
    ...
    char **argv;			/* Argument strings. */
{
	...
	Tcl_Eval(consoleInterp, Tcl_DStringValue(&dString));
    } else if (subString(argv[1], "show", 1)) {
	Tcl_DStringAppend(&dString, "wm deiconify . ", -1);
	Tcl_Eval(consoleInterp, Tcl_DStringValue(&dString));
    } else if (subString(argv[1], "eval", 1)) {
	if (argc == 3) {
	...
	Tcl_AppendResult(interp, "bad option \"", argv[1],
		"\": should be hide, show, title, or eval",
		(char *) NULL);
        result = TCL_ERROR;
    }
    ...
    char **argv;			/* Argument strings. */
{
    ...
    c = argv[1][0];
    length = strlen(argv[1]);
    otherInterp = (interp == info->interp) ? 
      info->consoleInterp : info->interp;
